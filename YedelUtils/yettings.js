...
const getIsAutoGG = () => { // Detect if AutoGG is on if (if it exists)
  autoGG = autoGGClass.INSTANCE
  if (autoGG instanceof autoGGClass) console.log('< Yedel > AutoGG detected')
  else {
    console.log('< Yedel > AutoGG not detected')
    return true
  }
  ...
  if (autoGGConfig.isModEnabled()) {
    console.log('< Yedel > AutoGG is on') 
    autoGGOn = true
    return true
  }
  else {
    console.log('< Yedel > AutoGG is off')
    return false
  }
}
...
const getAutoGGDelay = () => {
  if (autoGGOn) {
    delay = autoGGConfig.getAutoGGDelay()
    console.log(`< Yedel > AutoGG delay is ${delay}`) 
    return delay
  }
  else return 1 
}
...
class Yettings {
  ...
  @SelectorProperty({
    ...
    category: 'General',
    subcategory: 'Customization',
    options: ['Ping', 'Command', 'Tab', 'Stats (default)']
  })
  pingMethod = 3
  ...
  constructor() {
    ...
    this.addDependency('Give items to play again and leave the game', 'Bounty Hunting')
