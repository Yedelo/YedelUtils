...
import {killMessages} from '../../stuff/constants'
...
const strengthPlayers = new Map
const startStrengthPlayers = []
const endStrengthPlayers = []
...
const strength = (killed, num, killer, event) => {
  ...
  if (startStrengthPlayers.indexOf(killer) == -1) startStrengthPlayers.push(killer)
  endStrengthPlayers.pop(killer)

  strengthPlayers.set(killed, 0)
  startStrengthPlayers.pop(killed)
  endStrengthPlayers.pop(killed)
}
...
killMessages.forEach(msg => register('chat', strength).setCriteria(msg)) 
...
register('renderEntity', entity => {
  ...
  if (startStrengthPlayers.includes(entityName)) {
    if (!playerMP.canSeeEntity(entity)) return
    ...
    Tessellator.drawString('Strength', lastX + (currentX - lastX) * partialTicks, lastY + (currentY - lastY) * partialTicks + blackInc, lastZ + (currentZ - lastZ) * partialTicks, black, false, 0.05, false); // Shadow  
    Tessellator.drawString('Strength', lastX + (currentX - lastX) * partialTicks, lastY + (currentY - lastY) * partialTicks + colorInc, lastZ + (currentZ - lastZ) * partialTicks, darkRed, true, 0.05, false);
  }
  else if (endStrengthPlayers.includes(entityName)) {
    if (!playerMP.canSeeEntity(entity)) return
    partialTicks = Tessellator.getPartialTicks()
    lastX = entity.getLastX()
    lastY = entity.getLastY()
    ...
    colorInc = entity.isSneaking() ? 2.7 : 3.1
    Tessellator.drawString('Strength (?)', lastX + (currentX - lastX) * partialTicks, lastY + (currentY - lastY) * partialTicks + blackInc, lastZ + (currentZ - lastZ) * partialTicks, black, false, 0.05, false); // Shadow  
    Tessellator.drawString('Strength (?)', lastX + (currentX - lastX) * partialTicks, lastY + (currentY - lastY) * partialTicks + colorInc, lastZ + (currentZ - lastZ) * partialTicks, gold, true, 0.05, false);
  }
